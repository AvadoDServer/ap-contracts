...
contract APETHTest is Test {
    ...
    function testUpgradeability() public {
        ...
        APEth2 = APETHV2(payable(address(APEth)));
        assertTrue(APEth2.hasRole(bytes32(0), owner), "owner doesnet have DEFAULT_ADMIN_ROLE");
        vm.expectRevert();
        hoax(alice);
        APEth2.mint{value: 1 ether}();
        vm.prank(owner);
        APEth2.grantRole(EARLY_ACCESS, alice);
        vm.prank(alice);
        APEth2.mint{value: 1 ether}();
    }
