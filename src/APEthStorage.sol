...
error ACCOUNT_IS_NOT_GUARDIAN();
error ACCOUNT_IS_NOT_GUARDIAN_OR_APETH(address);
error MUST_COME_FROM_NEW_GUARDIAN();
error CAN_ONLY_BE_SET_ONCE();
error MUST_SET_TO_0X0_FIRST();
...
contract APEthStorage is IAPEthStorage{
    ...
    */
    modifier onlyGuardian() {
        if(msg.sender != guardian) revert ACCOUNT_IS_NOT_GUARDIAN();
        _;
    }
    ...
    */
    modifier onlyGuardianOrAPEth() {
        if(msg.sender != apEth && msg.sender != guardian) revert ACCOUNT_IS_NOT_GUARDIAN_OR_APETH(msg.sender);
        _;
    }
    ...
    function confirmGuardian() external override {
        ...
        if(msg.sender != newGuardian) revert MUST_COME_FROM_NEW_GUARDIAN();
    ...
    function getAPEth() external override view returns (address) {
        return apEth;
    }

    // Set the APEth contract address
    function setAPEth(address _newAddress) external override onlyGuardian{
        if(apEth != address(0)) revert CAN_ONLY_BE_SET_ONCE();
        apEth = _newAddress;
    }
    ...
    function burnKeys() external override onlyGuardian{
        ...
        if(address(0) != newGuardian) revert MUST_SET_TO_0X0_FIRST();
        // Store old guardian for event
        address oldGuardian = guardian;
        // delete guardian 
        delete guardian;
    ...
    /// @param _key The key for the record
    function setAddress(bytes32 _key, address _value) onlyGuardianOrAPEth override external {
        addressStorage[_key] = _value;
    }
    
    /// @param _key The key for the record
    function setUint(bytes32 _key, uint _value) onlyGuardianOrAPEth override external {
        uintStorage[_key] = _value;
    }

    /// @param _key The key for the record
    function setBool(bytes32 _key, bool _value) onlyGuardianOrAPEth override external {
        booleanStorage[_key] = _value;
    }

    /// @param _key The key for the record
    function deleteAddress(bytes32 _key) onlyGuardianOrAPEth override external {
        delete addressStorage[_key];
    }

    /// @param _key The key for the record - added for APEth ~ 0xWildhare
    function deleteUint(bytes32 _key) onlyGuardianOrAPEth override external {
        delete uintStorage[_key];
    }

    /// @param _key The key for the record
    function deleteBool(bytes32 _key) onlyGuardianOrAPEth override external {
        delete booleanStorage[_key];
    }

    /// @param _key The key for the record
    /// @param _amount An amount to add to the record's value  - 0xWildhare removed safeMath
    function addUint(bytes32 _key, uint256 _amount) onlyGuardianOrAPEth override external {
        uintStorage[_key] += _amount;
    }

    /// @param _key The key for the record
    /// @param _amount An amount to subtract from the record's value - 0xWildhare removed safeMath
    function subUint(bytes32 _key, uint256 _amount) onlyGuardianOrAPEth override external {
        uintStorage[_key] -= _amount;
    }
