...
contract APEthStorage is IAPEthStorage{
    ...
    */
    modifier onlyAPEth() {
        require(msg.sender == apEth, "Account is not apEth");
        _;
    }
    ...
    function setAPEth(address _newAddress) external override onlyGuardian{
        require(apEth == address(0), "address can onlly be set once");
        apEth = _newAddress;
    }
...
*/
    ...
    function addUint(bytes32 _key, uint256 _amount) onlyAPEth override external {
        uintStorage[_key] += _amount;
    }

    /// @param _key The key for the record
    /// @param _amount An amount to subtract from the record's value - 0xWildhare removed safeMath
    function subUint(bytes32 _key, uint256 _amount) onlyAPEth override external {
        uintStorage[_key] -= _amount;
    }
