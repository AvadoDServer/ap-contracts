...
contract APETH is Initializable, ERC20Upgradeable, AccessControlUpgradeable, ERC20PermitUpgradeable, UUPSUpgradeable {
     ...
     *
     */
    function stake(bytes calldata _pubKey, bytes calldata _signature, bytes32 _deposit_data_root) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // requires 32 ETH
     ...
     *
     */
    function callSSVNetwork(bytes memory data) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     *
     */
    function callEigenPod(bytes memory data) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // get address from storage
     ...
     *
     */
    function callEigenPodManager(bytes memory data) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // get address from storage
     ...
     *
     */
    function transferToken(address tokenAddress, address to, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
