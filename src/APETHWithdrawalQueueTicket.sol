    ...
    IAPETHWithdrawalQueueTicket
{
    ...
    IDateTimeContract constant dateTimeContract = IDateTimeContract(0x23d23d8F243e57d0b924bff3A3191078Af325101);
        ...
        returns (string memory)
    {
        ...
        (string memory available, string memory ethAmount, string memory ticketUnits) = _getAttributes(tokenId);
    ...
    function _getAttributes(uint256 tokenId) internal view returns (string memory, string memory, string memory) {
        string memory available = getTime(tokenIdToExitQueueTimestamp[tokenId]);
        string memory ethAmount = getEthAmount(tokenIdToExitQueueExitAmount[tokenId]);
        string memory ticketUnits = readyToWithdraw[tokenId] ? " ETH" : " APETH";
        return (available, ethAmount, ticketUnits);
    }
    ...
    function getTime(uint256 timestamp) public view returns (string memory) {
        ...
        string memory leadingZeroMin = minute < 10 ? "0" : "";
        return string(
            abi.encodePacked(
                day.toString(),
                "-",
