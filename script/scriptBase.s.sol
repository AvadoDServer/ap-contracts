...
contract ScriptBase is Script {
    ...
    bytes32 private constant DEFAULT_SALT =
            ...
            Create2.computeAddress(
                ...
                keccak256(
                    abi.encodePacked(
                        ...
                        abi.encode(implementation, encodeInitializer(config))
                    )
                ),
                FACTORY
            );
    ...
    function deployProxy(
        ...
        ERC1967Proxy proxy = new ERC1967Proxy{salt: getSalt(config)}(
            implementation,
            encodeInitializer(config)
        );
    ...
    function getEarlyDepositAddress() public view returns (address) {
        ...
        string memory path = string.concat(
            root,
            "/broadcast/deployEarlyDepositOnly.s.sol/",
            chainId,
            "/run-latest.json"
        );
        string memory json = vm.readFile(path);
    ...
    function getAPEthEarlyDeposits(
        address addr
    ) public pure returns (APEthEarlyDeposits) {
